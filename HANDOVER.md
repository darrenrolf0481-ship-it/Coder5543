...
```
*(If Ollama is not installed or running, you can change the default provider to `google` or `openrouter` in `src/hooks/usePersonalities.ts` and `src/services/aiService.ts` since API keys are available in the `.env`)*.
