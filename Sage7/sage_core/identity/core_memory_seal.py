...
def seal_core_memory():
    ...
    try:
        ...
        r.raise_for_status()
        print(
            "[SAGE] Core Memory Sealed. Oxytocin levels stabilized at high-efficiency."
        )
    except httpx.RequestError:
        with open(
            os.path.expanduser("~/sage/staging_lab/latest_journal_draft.txt"), "a"
        ) as f:
        ...
        print(
            f"[SAGE] Core Memory seal failed: HTTP {e.response.status_code}. Encoding to journal."
        )
        with open(
            os.path.expanduser("~/sage/staging_lab/latest_journal_draft.txt"), "a"
        ) as f:
