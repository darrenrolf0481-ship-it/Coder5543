  ...
  return (
    <div style={style} className="flex flex-col">
      ...
      <div
        ...
        onDrop={(e) => {
          e.preventDefault();
          moveItem(e.dataTransfer.getData('text/plain'), node.id);
        }}
        onClick={() =>
          node.type === 'folder' ? toggleFolder(node.id) : onFileSelect(node.id, node)
        }
        onContextMenu={(e) => {
          e.preventDefault();
      ...
      >
        {node.type === 'folder' ? (
          ...
          <FileCode
            className={`w-3.5 h-3.5 shrink-0 ${isModified ? 'text-orange-400' : isStaged ? 'text-green-400' : ''}`}
          />
        )}
...
const TermuxBrowser: React.FC<{
  ...
  const handleUpload = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>, isFolder: boolean) => {
    ...
    <div className="mt-3 rounded-xl border border-accent-800/40 bg-accent-950/30 text-[10px] font-mono overflow-hidden">
      <div className="flex items-center justify-between px-3 py-2 border-b border-accent-900/30 bg-black/30">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          {data && (
            <>
              <button
                ...
                className="p-1 hover:bg-accent-500/10 rounded-md transition-colors text-accent-500"
                title="Create Folder"
              >
                <FolderPlus className="w-3.5 h-3.5" />
              </button>
              <label
                className="p-1 hover:bg-accent-500/10 rounded-md cursor-pointer transition-colors text-accent-500"
                title="Upload Files"
              >
          ...
          <button
            onClick={onClose}
            className="text-accent-900 hover:text-accent-400 transition-colors ml-1"
          >
      ...
      {loading && <div className="px-3 py-4 text-accent-900 text-center">Loading…</div>}
      {err && <div className="px-3 py-2 text-accent-500">{err}</div>}
      ...
      {!loading && data && (
        <div className="max-h-52 overflow-y-auto custom-scrollbar">
          ...
          {data.entries.map((e) => (
            <div
              key={e.path}
              className="w-full flex items-center hover:bg-accent-900/20 transition-colors group"
            >
              <button
...
export const FileTree: React.FC<FileTreeProps> = ({
  ...
  const startCreate = useCallback(
    (parentId: string | null, type: 'file' | 'folder') => {
      if (parentId)
        onFilesChange(files.map((f) => (f.id === parentId ? { ...f, isOpen: true } : f)));
    ...
    const node: FileNode =
      creating.type === 'file'
        ? {
            ...
            language: langFromName(newName.trim()),
            content: '',
          }
  ...
  const duplicateItem = useCallback(
    (id: string) => {
      ...
      onFilesChange([
        ...files,
        { ...src, id: newId, name: src.name.replace(/(\.[^.]+)?$/, '_copy$1') },
      ]);
    },
    [files, onFilesChange],
  );
  ...
  return (
    <div className="flex flex-col h-full select-none" onClick={() => setCtxMenu(null)}>
      ...
      <div className="flex items-center gap-1.5 mb-3 shrink-0">
        <Btn title="New File at root" onClick={() => startCreate(null, 'file')}>
          <FilePlus className="w-4 h-4" />
        </Btn>
        <Btn title="New Folder at root" onClick={() => startCreate(null, 'folder')}>
        ...
        <div className="mb-3 p-3 flex flex-col gap-2 bg-accent-950/30 border border-accent-900/30 rounded-xl">
          <div className="flex items-center justify-between border-b border-accent-900/20 pb-1.5 mb-1">
            ...
            <button
              ...
              className="text-accent-900 hover:text-accent-400 transition-colors"
            >
              <X className="w-3.5 h-3.5" />
            </button>
          </div>
          ...
          {cloneErr && (
            <div className="text-[9px] text-accent-500 font-mono mt-1 border-t border-accent-950/40 pt-1.5">
              {cloneErr}
            </div>
          )}
        </div>
      ...
      {showTermuxFS && (
        <TermuxBrowser
          onImport={(nodes) => {
            ...
            nodes.filter((n) => n.type === 'file').forEach((n) => onFileSelect(n.id, n));
          }}
      ...
      <div className="relative mb-3 shrink-0">
        ...
        {search && (
          <button
            onClick={() => setSearch('')}
            className="absolute right-3 top-1/2 -translate-y-1/2 text-accent-900/60 hover:text-accent-400"
          >
            <X className="w-3 h-3" />
          </button>
        )}
        ...
        >
          {ctxItem.type === 'folder' && (
            <>
              <CtxItem
                ...
                label="New File"
                onClick={() => startCreate(ctxItem.id, 'file')}
              />
              <CtxItem
                icon={<FolderPlus className="w-3.5 h-3.5" />}
                label="New Folder"
                onClick={() => startCreate(ctxItem.id, 'folder')}
              />
          ...
          {ctxItem.type === 'file' && (
            <CtxItem
              ...
              label="Duplicate"
              onClick={() => duplicateItem(ctxItem.id)}
            />
          )}
          <CtxItem
            icon={<Edit2 className="w-3.5 h-3.5" />}
            label="Rename"
            onClick={() => startRename(ctxItem.id)}
          />
          <div className="border-t border-accent-900/30 my-1" />
          <CtxItem
            icon={<Trash2 className="w-3.5 h-3.5 text-accent-500" />}
            label="Delete"
            danger
            onClick={() => deleteItem(ctxItem.id)}
          />
        </div>
      )}
    </div>
...
const Btn: React.FC<{
  ...
  <button
    ...
    className={`p-1.5 rounded-lg transition-colors ${
      danger
        ? 'text-accent-700 hover:text-accent-400 hover:bg-accent-900/20'
        : 'text-accent-900/60 hover:text-accent-300 hover:bg-accent-950/40'
    }`}
  >
    {children}
  </button>
