...
const renderContent = (line: string): React.ReactNode => {
  if (line.startsWith('$ ')) {
    return (
      <>
        ...
        <span className="text-accent-300 font-bold">{line.substring(2)}</span>
      </>
    );
  }
  ...
  while ((match = regex.exec(line)) !== null) {
    ...
    const m = match[0];
    let cls = 'text-accent-100/60';
    if (m === '[ERROR]') cls = 'text-accent-500 font-black bg-accent-950/50 px-1 rounded';
  ...
  return parts.length > 0 ? <>{parts}</> : <span className="text-accent-100/60">{line}</span>;
};
