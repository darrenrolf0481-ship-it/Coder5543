...
export const CommitModal: React.FC<CommitModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8 bg-black/90 backdrop-blur-2xl animate-in fade-in duration-300">
      <div className="w-full max-w-lg bg-[#0d0404] border border-red-900/30 rounded-[30px] shadow-[0_0_100px_rgba(185,28,28,0.2)] overflow-hidden flex flex-col">
        ...
        <div className="p-6 space-y-4">
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full bg-black/60 border border-red-900/40 rounded-xl px-5 py-4 text-sm text-red-100 focus:border-red-500/60 outline-none transition-all"
              aria-label="Commit message input"
            />
          </div>
        </div>
        <div className="p-6 border-t border-red-900/20 bg-black/40 flex justify-end">
          <button
            onClick={confirmGitCommit}
            disabled={!commitMessage.trim()}
            className="w-full flex items-center justify-center gap-2 py-4 bg-red-700 hover:bg-red-600 text-white rounded-xl font-black text-xs uppercase tracking-[0.2em] shadow-lg active:scale-95 transition-all disabled:opacity-50"
