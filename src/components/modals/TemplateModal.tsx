            ...
            {(Object.keys(PROJECT_TEMPLATES) as Array<keyof typeof PROJECT_TEMPLATES>).map(
              (key) => (
                ...
                >
                  ...
                  <div className="space-y-1 md:space-y-2">
                    <h4 className="text-sm md:text-lg font-black text-red-100 uppercase tracking-tight group-hover:text-red-500 transition-colors">
                      {key.replace('-', ' ')}
                    </h4>
                    <p className="text-[9px] md:text-xs text-red-900 font-bold leading-relaxed uppercase tracking-wider line-clamp-3">
                  ...
                  <div className="flex flex-wrap gap-1.5 md:gap-2 pt-2 md:pt-4">
                    {PROJECT_TEMPLATES[key].tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-2 md:px-3 py-1 bg-red-950/20 border border-red-900/20 rounded-full text-[7px] md:text-[9px] font-black text-red-400 uppercase tracking-widest"
                      >
                        {tag}
                      </span>
                    ))}
