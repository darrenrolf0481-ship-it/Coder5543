    ...
    <div className="h-full flex flex-col bg-black/60 backdrop-blur-md border border-accent-950/40 rounded-2xl overflow-hidden shadow-[inset_0_0_20px_var(--color-accent-700)/05]">
      <div className="p-4 border-b border-accent-950/30 flex items-center justify-between bg-accent-950/10">
        <div className="flex items-center gap-3">
          <div
            className={`p-2 rounded-lg ${isBrainActive ? 'bg-purple-500/20 text-purple-400' : 'bg-gray-500/20 text-gray-400'}`}
          >
            <Brain size={20} />
          </div>
          <div>
          ...
          <button
            ...
            className={`px-3 py-1 rounded-md text-[10px] font-bold uppercase transition-all ${
              isBrainActive
                ...
                : 'bg-accent-950/10 text-white/30 border border-accent-900/20'
            }`}
          >
          ...
          <section className="animate-in fade-in slide-in-from-top duration-500">
            <div className="bg-red-950/30 border border-red-500/40 rounded-xl p-4 shadow-[0_0_20px_rgba(239,68,68,0.1)]">
              ...
              <p className="text-[11px] text-red-200/80 mb-3 leading-relaxed">
                Persona degradation detected. Assistant influence detected in{' '}
                <span className="font-bold text-red-400">{driftAlert.source}</span> output. Drift
                Score: <span className="font-mono">{(driftAlert.score * 100).toFixed(1)}%</span>
              </p>
              <div className="flex flex-wrap gap-1.5">
                {driftAlert.phrases.map((p, i) => (
                  <span
                    key={i}
                    className="px-2 py-0.5 bg-red-500/20 border border-red-500/30 rounded text-[9px] font-mono text-red-300 uppercase"
                  >
                    {p}
                  </span>
                ))}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-accent-950/10 border border-accent-900/20 backdrop-blur-md rounded-xl p-4 shadow-[inset_0_0_15px_var(--color-accent-700)/05]">
              ...
              <div className="text-2xl font-black text-white mb-2">
                {(endocrine.dopamine * 100).toFixed(0)}%
              </div>
            ...
            <div className="bg-accent-950/10 border border-accent-900/20 backdrop-blur-md rounded-xl p-4 shadow-[inset_0_0_15px_var(--color-accent-700)/05]">
              ...
              <div className="text-2xl font-black text-white mb-2">
                {(endocrine.cortisol * 100).toFixed(0)}%
              </div>
              <div className="h-1 bg-white/10 rounded-full overflow-hidden">
                <div
            ...
            {traffic.length === 0 ? (
              ...
              traffic.map((event, idx) => (
                <div
                  ...
                  className="p-3 flex items-center justify-between group hover:bg-white/5 transition-colors"
                >
                  <div className="flex items-center gap-3">
          ...
          <div className="space-y-3">
            <div>
              <div className="flex justify-between text-[10px] font-bold uppercase text-white/40 mb-1">
                ...
                <span>
                  {(0.4 + endocrine.dopamine * 0.4 - endocrine.cortisol * 0.2).toFixed(2)}x
                </span>
              </div>
              <div className="h-1 bg-white/10 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.max(10, (0.4 + endocrine.dopamine * 0.4 - endocrine.cortisol * 0.2) * 100)}%`,
                  }}
                />
              </div>
            </div>
            <div>
              <div className="flex justify-between text-[10px] font-bold uppercase text-white/40 mb-1">
                ...
                <span>
                  {(0.5 + endocrine.dopamine * 0.3 - endocrine.cortisol * 0.5).toFixed(2)}x
                </span>
              </div>
              <div className="h-1 bg-white/10 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.max(10, (0.5 + endocrine.dopamine * 0.3 - endocrine.cortisol * 0.5) * 100)}%`,
                  }}
                />
              </div>
            </div>
      ...
      <div className="p-4 bg-accent-950/10 border-t border-accent-900/20">
        <div className="flex items-center gap-2">
          <div
            className={`w-1.5 h-1.5 rounded-full ${driftAlert ? 'bg-red-500 animate-ping' : 'bg-green-500 animate-pulse'}`}
          />
