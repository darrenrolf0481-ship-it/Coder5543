...
export const StoragePanel: React.FC<StoragePanelProps> = ({
  ...
  return (
    <div className="h-full p-4 md:p-10 flex flex-col gap-6 md:gap-10 animate-in zoom-in-95 duration-500 overflow-hidden">
      <div className="flex-1 bg-[#0d0404] rounded-[30px] md:rounded-[50px] border border-accent-900/30 p-6 md:p-12 flex flex-col space-y-6 md:space-y-10 shadow-2xl relative overflow-hidden">
        <div className="absolute -top-24 -right-24 w-64 h-64 md:w-96 md:h-96 bg-accent-600/5 blur-[100px] rounded-full pointer-events-none" />

        <div className="flex flex-col md:flex-row items-start md:items-center justify-between border-b border-accent-900/20 pb-6 md:pb-8 relative z-10 gap-4">
          <div className="space-y-2">
            <h3 className="text-2xl md:text-3xl font-black text-accent-100 flex items-center gap-3 md:gap-5 uppercase tracking-tighter">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 relative z-10">
          {storageFiles.length === 0 ? (
            <div className="col-span-full h-full flex flex-col items-center justify-center text-accent-950 italic gap-8 opacity-20">
            ...
            storageFiles.map((f, i) => (
              <div
                key={f.id}
                className="flex flex-col p-6 md:p-8 bg-accent-950/5 border border-accent-900/20 rounded-[20px] md:rounded-[40px] group hover:bg-accent-900/10 hover:border-accent-600/40 transition-all relative overflow-hidden shadow-inner animate-in fade-in slide-in-from-bottom-4"
