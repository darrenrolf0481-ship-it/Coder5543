        ...
        <div className="hidden lg:flex w-72 flex-col shrink-0">
          <div className="flex-1 code-editor-bg rounded-[40px] border border-accent-900/30 p-6 flex flex-col gap-6 overflow-y-auto custom-scrollbar shadow-2xl">
            <div className="space-y-1">
              ...
              <p className="text-[9px] text-accent-900 font-black tracking-[0.3em] uppercase">
            ...
            <div className="space-y-4">
              <h4 className="text-[10px] font-black text-accent-800 uppercase tracking-[0.4em]">
                System Status
              </h4>
              <div className="space-y-3">
                <div className="p-4 bg-accent-950/10 rounded-2xl border border-accent-900/10">
                  <p className="text-[9px] text-accent-900 font-black uppercase mb-2">
                    Local Inference
                  </p>
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 md:space-y-8 custom-scrollbar">
                {chatMessages.length === 0 && (
                  <div className="h-full flex flex-col items-center justify-center text-center p-4 opacity-40">
                    ...
                    <p className="text-[11px] font-black uppercase tracking-[0.2em] text-accent-400 mb-1">
                      Neural Chat Ready
                    </p>
                    <p className="text-[10px] text-accent-700 uppercase tracking-widest">
                ...
                {chatMessages.map((msg, i) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`max-w-[85%] rounded-3xl p-5 text-[13px] leading-relaxed ${
                          msg.role === 'user'
                        ...
                        {msg.type === 'image' ? (
                          <div className="space-y-4">
                            <img
                              src={msg.url}
                              alt="Generated"
                              className="w-full rounded-xl border border-accent-900/30"
                            />
                            <p className="text-[10px] font-mono text-accent-400 opacity-70">
                              {msg.text}
                            </p>
                          </div>
                        ) : (
                            ...
                            {msg.role === 'ai' && (showSave || codeBlocks.length > 0) && (
                              <div className="mt-4 pt-3 border-t border-accent-900/20 space-y-2">
                                ...
                                {codeBlocks.map((block, bi) => (
                                  <div
                                    key={bi}
                                    className="rounded-lg overflow-hidden border border-accent-900/20"
                                  >
                                    <div className="flex items-center justify-between px-3 py-1.5 bg-black/40 border-b border-accent-900/20">
                                      <span className="text-[9px] font-mono text-accent-700 uppercase tracking-widest">
                                        {block.lang}
                                      </span>
                                    ...
                                    <pre className="px-3 py-2 text-[10px] font-mono text-accent-200/70 overflow-x-auto max-h-32 bg-black/20 whitespace-pre">
                                      {block.code}
                                    </pre>
                                  </div>
                                ))}
            ...
            <div className="flex-1 p-6 md:p-12 space-y-6 md:space-y-10 overflow-y-auto custom-scrollbar">
              <div className="flex items-center justify-between">
                <div className="space-y-2">
                  <h3 className="text-2xl font-black text-accent-100 uppercase tracking-tighter">
                    Neural RAG Database
                  </h3>
                  <p className="text-sm text-accent-900 font-bold tracking-widest">
                    Inject specialized datasets for context-aware inference.
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {tnKnowledgePacks.map((pack) => (
                  <div
                    key={pack.id}
                    className="p-6 md:p-8 bg-accent-950/5 border border-accent-900/20 rounded-[20px] md:rounded-[32px] group hover:bg-accent-900/10 transition-all relative overflow-hidden"
                  >
                    {pack.status === 'indexing' && (
                      <div className="absolute inset-0 bg-accent-950/40 backdrop-blur-[2px] z-10 flex items-center justify-center">
                        <div className="flex flex-col items-center gap-3 w-full px-8">
                          ...
                          <span className="text-[10px] font-black text-accent-500 uppercase tracking-[0.3em]">
                            Indexing Neural Vectors...
                          </span>
                    ...
                    <div className="flex items-center gap-6 mb-6">
                      ...
                      <div className="flex-1">
                        <p className="text-[15px] font-black text-accent-100 uppercase tracking-tight">
                          {pack.name}
                        </p>
                        <p className="text-[10px] uppercase tracking-[0.3em] text-accent-800 font-black mt-2">
                          {pack.size} • {pack.status}
                        </p>
                      </div>
                    </div>
                    <div className="flex gap-3">
                      <button className="flex-1 py-3 bg-accent-900/20 text-[10px] font-black uppercase text-accent-700 rounded-xl">
                        Re-Index
                      </button>
                      <button className="p-3 text-accent-900 hover:text-accent-500 transition-all">
          ...
          {tnModule === 'vault' && (
            <div className="flex-1 flex flex-col h-full overflow-hidden">
              {!isVaultUnlocked ? (
                <div
                  key="locked"
                  className="flex-1 flex flex-col items-center justify-center p-6 md:p-12 space-y-8 md:space-y-12 text-center transition-all overflow-y-auto custom-scrollbar"
                >
                  ...
                  {vaultStep === 'initial' && !isBiometricVerifying && (
                    <div className="flex flex-col gap-4 w-full max-w-xs">
                      <button
                        ...
                        className="w-full py-5 bg-accent-700 text-white rounded-[32px] font-black text-xs uppercase tracking-[0.4em] shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-3"
                      >
                      ...
                      <button
                        ...
                        className="w-full py-5 bg-transparent border border-accent-900/30 text-accent-600 rounded-[32px] font-black text-xs uppercase tracking-[0.4em] hover:bg-accent-900/10 active:scale-95 transition-all"
                      >
                        Use PIN Code
                      </button>
                    </div>
                    ...
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
                      <div className="flex gap-4 justify-center">
                        {[0, 1, 2, 3].map((i) => (
                          <div
                            key={i}
                            className={`w-4 h-4 rounded-full border-2 transition-all duration-300 ${vaultPin.length > i ? 'bg-accent-600 border-accent-600 scale-125 shadow-[0_0_10px_var(--color-accent-600)/0.5]' : 'border-accent-900/50'}`}
                          />
                        ))}
                      </div>
                      ...
                      <button
                        ...
                        className="text-[10px] font-black text-accent-900 uppercase tracking-[0.3em] hover:text-accent-600 transition-colors"
                      >
                        Cancel Verification
                      </button>
                    </div>
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="space-y-2">
                      ...
                      <p className="text-[10px] text-accent-900 font-black uppercase tracking-[0.3em]">
                        Secure Session Active • AES-256-GCM
                      </p>
                    </div>
                    <button
                      onClick={() => setIsVaultUnlocked(false)}
                      className="px-6 py-3 bg-accent-950/30 border border-accent-900/30 text-accent-600 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-accent-900/20 transition-all"
                    >
                      Lock Vault
                    </button>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                    {(vaultMemories.length > 0
                      ? vaultMemories.map((m) => ({
                          ...
                          desc: m.intent
                            ? m.intent.length > 100
                              ? m.intent.substring(0, 100) + '...'
                              : m.intent
                            : 'Encrypted memory fragment',
                      ...
                      <div
                        key={item.id}
                        className="p-6 md:p-8 bg-accent-950/5 border border-accent-900/20 rounded-[20px] md:rounded-[40px] space-y-4 group hover:border-accent-600/30 transition-all cursor-pointer animate-in fade-in slide-in-from-left-4"
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <span
                            className={`text-[10px] font-black uppercase tracking-widest ${item.size.includes('+') ? 'text-yellow-400' : item.size.includes('-') ? 'text-accent-500' : 'text-accent-900'}`}
                          >
                            {item.size}
                          </span>
                        </div>
                        <div className="space-y-2">
                          <h4 className="text-lg font-black text-accent-100 uppercase tracking-tight">
                            {item.title}
                          </h4>
                        ...
                        <div className="pt-4 flex items-center justify-between border-t border-accent-900/10">
                          <span className="text-[9px] font-black text-accent-950 uppercase tracking-widest">
                            {item.date}
                          </span>
          ...
          {tnModule === 'help' && (
            <div className="flex-1 p-6 md:p-12 space-y-8 md:space-y-12 overflow-y-auto custom-scrollbar">
              <div className="space-y-4">
                ...
                <p className="text-sm text-accent-900 font-bold tracking-widest uppercase">
                  Understanding the ToolNeuron Ecosystem
                </p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                {[
                  ...
                  <div
                    key={i}
                    className="p-6 md:p-8 bg-accent-950/5 border border-accent-900/20 rounded-[20px] md:rounded-[40px] space-y-4"
                  >
