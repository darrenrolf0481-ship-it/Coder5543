    ...
    const runShellCmd = async (shellCmd: string) => {
      ...
      if (/^cd(\s|$)/.test(shellCmd.trim())) {
        try {
          ...
          if (data.stderr)
            setTerminalOutput((prev: string[]) => [...prev, `[ERROR] ${stripAnsi(data.stderr)}`]);
        } catch {
    ...
    if (finalCmd === 'clear') {
      ...
      if (action === 'list') {
        ...
        try {
          ...
          if (data.success && Array.isArray(data.tools)) {
            ...
            setTerminalOutput((prev: string[]) => [
              ...prev,
              `[ERROR] Failed to fetch tools: ${data.error || 'Unknown error'}`,
            ]);
          }
        } catch {
      ...
      if (action === 'enable' || action === 'disable') {
        ...
        if (!toolName) {
          setTerminalOutput((prev: string[]) => [
            ...prev,
            `[ERROR] Usage: mcp ${action} <tool_name>`,
          ]);
          return;
        }
        setIsAiProcessing(true);
        try {
          ...
          if (data.success) {
            setTerminalOutput((prev: string[]) => [
              ...prev,
              `[OK] Tool '${toolName}' successfully ${action}d.`,
            ]);
          } else {
            setTerminalOutput((prev: string[]) => [
              ...prev,
              `[ERROR] Failed to toggle tool: ${data.error || 'Unknown error'}`,
            ]);
          }
        } catch {
          setTerminalOutput((prev: string[]) => [...prev, '[ERROR] MCP service unreachable.']);
      ...
      if (action === 'call') {
        ...
        if (!toolName) {
          setTerminalOutput((prev: string[]) => [
            ...prev,
            '[ERROR] Usage: mcp call <tool_name> [json_arguments]',
          ]);
          return;
        }
        ...
        try {
          ...
          const data = await res.json();
          if (data.success) {
            const lines = [`[OK] Response from ${toolName}:`, JSON.stringify(data.result, null, 2)];
            setTerminalOutput((prev: string[]) => [...prev, ...lines]);
          } else {
            setTerminalOutput((prev: string[]) => [
              ...prev,
              `[ERROR] Tool call failed: ${data.error || 'Unknown error'}`,
            ]);
          }
        } catch {
          setTerminalOutput((prev: string[]) => [...prev, '[ERROR] MCP service unreachable.']);
