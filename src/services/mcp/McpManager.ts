...
export class McpManager {
  ...
  private async loadSerenaServer(cwd: string) {
    ...
    const child = spawn(
      'uv',
      ['run', 'serena', 'start-mcp-server', '--transport', 'stdio', '--project-from-cwd'],
      {
          ...
          for (let i = 0; i < lines.length - 1; i++) {
            ...
            try {
              ...
              if (stage === 'init' && res.id === 'init_1') {
                ...
                child.stdin.write(
                  JSON.stringify({ jsonrpc: '2.0', id: 'init_list', method: 'tools/list' }) + '\n',
                );
  ...
  async callTool(name: string, params: any, id: string | number) {
    ...
    if (instance.type === 'library') {
      const response = await instance.handle.handleMessage(
        JSON.stringify({ jsonrpc: '2.0', method: 'tools/call', params, id }),
      );
      ...
      return {
        jsonrpc: '2.0',
        id,
        result: { content: [{ type: 'text', text: `Mock result for ${name}` }] },
      };
    }
