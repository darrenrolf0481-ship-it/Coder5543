...
const SKILL_FILE_PATTERNS: Record<string, string[]> = {
  'threat-modeling': ['auth', 'login', 'session', 'permission', 'rbac', 'oauth', 'jwt', 'crypto', 'password', 'token'],
  'vulnerability-scanning': ['auth', 'input', 'validate', 'sanitize', 'sql', 'query', 'exec', 'eval', 'deserialize'],
  'secure-code-review': ['.ts', '.tsx', '.js', '.jsx', '.py', '.go', '.rs', '.java'],
  'dependency-audit': ['package.json', 'package-lock', 'yarn.lock', 'pnpm-lock', 'requirements.txt', 'pyproject.toml', 'Cargo.toml', 'go.mod', 'go.sum'],
  'architecture-review': ['architecture', 'adr', 'design', 'README', 'docs', 'CONTRIBUTING'],
  'trade-off-analysis': ['architecture', 'adr', 'design', 'README'],
  'api-design': ['api', 'route', 'router', 'controller', 'endpoint', 'schema', 'openapi', 'swagger', 'handler'],
  'database-design': ['schema', 'migration', 'model', 'repository', 'prisma', 'sql', 'entity', 'orm'],
...
function scoreFileForAgent(file: RepoFile, agent: SwarmAgent): number {
  ...
  if (agent.agentDefinitionId?.includes('security')) {
    ...
    if (pathLower.includes('env') || pathLower.includes('secret') || pathLower.includes('config')) score += 4;
  }
  ...
  if (agent.agentDefinitionId?.includes('backend') || agent.skills.includes('api-design')) {
    if (pathLower.includes('route') || pathLower.includes('api') || pathLower.includes('controller')) score += 4;
  }

  // Universal high-value files
  if (nameLower === 'readme.md') score += 6;
  if (nameLower === 'package.json' || nameLower === 'pyproject.toml' || nameLower === 'cargo.toml') score += 8;
  if (nameLower.startsWith('dockerfile') || nameLower.endsWith('.yml') || nameLower.endsWith('.yaml')) score += 2;
...
export function buildRepoContextForAgent(
  ...
  for (const repo of assigned) {
    ...
    repoSection.push(`## Repository: ${repo.owner}/${repo.repo}${repo.branch ? ` (${repo.branch})` : ''}`);
